...
const Player = ({ player, currentPlayer, giveDrink, participant }) => {
  ...
  const GiveDrinkButton = (
    <Tooltip placement="top" title={`Give ${player.user.username} a drink`}>
      <Icon onClick={() => giveDrink(player.user._id)} type="coffee" />
    </Tooltip>
  );
  ...
  const MuteButton = participant ? (
    <Tooltip
      placement="top"
      title={`${muted ? "Unmute" : "Mute"} ${player.user.username}`}
    >
      <Icon
  ...
  const AudioVideo =
    participant !== null ? (
      ...
      <img width="100%" height="100%" src={NoVideoImage} alt="no video" />
    );

  const cardStyle = {
    border: `8px solid ${
      player.user._id === currentPlayer ? "#1890ff" : "#000000"
    }`,
    borderRadius: "5px",
    width: "100%",
    height: "100%"
  ...
  const title =
    player.politicalRank !== undefined ? (
